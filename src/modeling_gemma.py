...
# 논문 원본은 인접 쌍(q_i, q_{i+1})을 회전하지만, 이 구현은 전반부/후반부 쌍(q_i, q_{i+d/2})을 회전한다. (d=head_dim)
def rotate_half(x):
...
class GemmaRotaryEmbedding(nn.Module):
    ...
    def forward(self, x, position_ids, seq_len=None):
        ...
        with torch.autocast(device_type=device_type, enabled=False):
            ...
            # this part is different from the original paper.
...
# GemmaForCausalLM = GemmaModel + LM head
...
# Q: why is called conditional generation?
# A: because the model generates text conditioned on image inputs.
class PaliGemmaForConditionalGeneration(nn.Module):
    def __init__(self, config: PaliGemmaConfig):
        super().__init__()
        self.config = config
        self.vision_tower = SiglipVisionModel(config.vision_config)
        # convert the size of the embedding output by the vision encoder into the size of the embedding of each text token
        # so, they can be concatenated with text token embeddings
    ...
    def _merge_input_ids_with_image_features(
        self,
        image_features: torch.Tensor,  # (batch_size, image_seq_len, hidden_size)
        ...
        # insert image embeddings. we can't use torch.where because the sequence length of scaled_image_features is not equal to that of final_embedding
    ...
    def forward(
        ...
        # (batch_size, num_patches, embed_dim) -> (batch_size, image_seq_len, hidden_size)
