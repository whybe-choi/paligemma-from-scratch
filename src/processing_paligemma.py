...
def process_images(
    ...
    # Move the channel dimension to the first dimension. The model expects images in the format (num_channels, height, width)
...
class PaliGemmaProcessor:
    ...
    def __init__(
        ...
        # PaliGemma는 Gemma의 tokenizer를 사용하지만, 해당 tokenizer는 이미지를 위한 스페셜 토큰이 없음.
    ...
    def __call__(
        ...
        # 각 이미지에 대해서 resize, rescaling, normalization 수행
        ...
        # Convert the list of numpy arrays to a single numpy array with shape (batch_size, num_channels, height, width)
